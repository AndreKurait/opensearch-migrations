...
CHUNK_SIZE = 5 * 1024 * 1024  # 5MB characters per chunk
INITIAL_DOC_SIZE = 9 * 1024 * 1024  # 9MB in bytes
...
def identify_fields_to_chunk(doc: Dict[str, Any]) -> List[Tuple[str, str]]:
    """Identifies string fields that need to be chunked to get document size under MAX_DOC_SIZE."""
    doc_size = calculate_document_size(doc)
    logger.info(f"Original document size: {doc_size} bytes")

    if doc_size <= INITIAL_DOC_SIZE:
    ...
    for field_name, field_value in string_fields:
        ...
        if calculate_document_size(working_doc) <= INITIAL_DOC_SIZE:
            break
...
def update_field_with_chunks(env: Environment, index: str, doc_id: str,
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        payload = {
            "script": {
                "lang": "painless",
                "source": "if(ctx._source[params.field]==null){ctx._source[params.field]=\"\";}ctx._source[params.field]+=params.chunk;",
                "params": {
                    "field": field_name,
                    "chunk": chunk
                }
...
def main(env: Environment, args: argparse.Namespace) -> None:
    ...
    try:
        ...
        fields_to_chunk = identify_fields_to_chunk(doc)
        ...
        if not fields_to_chunk:
            ...
            for field_name, field_value in fields_to_chunk:
                update_field_with_chunks(env, dst_index, dst_id, field_name, field_value, DST_CLUSTER)
