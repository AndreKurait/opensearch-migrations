...
impl<'r, R: CommandRunner> App<'r, R> {
    ...
    pub fn cfn_deploy_or_skip(&mut self, template_file: &str) -> Result<()> {
        ...
        // Stream: `cloudformation deploy` blocks for ~15 min creating VPC+EKS
        // with no output otherwise — live lines tell the operator it's alive.
        let out = self.runner.run_streamed("aws", &args, &[], "cfn");
        if !out.success() {
    ...
    pub fn helm_install_or_upgrade(&mut self, chart: &str, values_files: &[String]) -> Result<()> {
        ...
        // Stream helm: `--wait` blocks up to 25m while resources converge —
        // live output is essential to see progress (and any error) in CI.
        let mut helm_full: Vec<&str> = Vec::new();
        if !kube_ctx.is_empty() {
            helm_full.push("--kube-context");
            helm_full.push(&kube_ctx);
        }
        helm_full.extend(args.iter().map(String::as_str));
        ...
        let full = self.runner.run_streamed("helm", &helm_full, &[], "helm");
        if !full.success() {
    ...
    pub fn resolve_cfn_template(&self) -> Result<String> {
        ...
        if self.is_build() {
            ...
            let out = self.runner.run("curl", &["-fsSL", "-o", &dest, &url]);
            if !out.success() || !std::path::Path::new(&dest).is_file() {
                return Err(Error::die(format!(
                    "failed to download CFN template from {url}"
                )));
            }
            Ok(dest)
        }
    ...
    pub fn resolve_chart(&self) -> Result<(String, Vec<String>)> {
        ...
        if !out.success() || !std::path::Path::new(&dest).is_file() {
            return Err(Error::die(format!(
                "failed to download helm chart from {url}"
            )));
        }
...
mod tests {
    ...
    fn cfn_runs_deploy_when_stack_absent() {
        ...
        let deploy = r
            .calls_to("aws")
            .into_iter()
            .find(|c| c.joined().contains("deploy"))
            .unwrap();
        assert!(deploy
            .joined()
            .contains("--stack-name MigrationAssistant-ma"));
        assert!(deploy.joined().contains("CAPABILITY_NAMED_IAM"));
        assert!(deploy.joined().contains("Stage=ma"));
    }
    ...
    fn helm_install_assembles_mirrored_flags_and_waits() {
        ...
        let upgrade = r
            .calls_to("helm")
            .into_iter()
            .find(|c| c.joined().contains("upgrade --install"))
            .unwrap();
        let j = upgrade.joined();
