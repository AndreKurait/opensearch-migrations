...
public class ReactorHttpClient implements AbstractedHttpClient {
    ...
    public AbstractHttpResponse makeRequest(String method, String path, Map<String, String> headers, String payload)
            throws IOException {
        var requestSender = client.request(HttpMethod.valueOf(method))
                .uri("/"+path);
        BiFunction<HttpClientResponse, ByteBufMono, Mono<Response>> responseWrapperFunction =
                (response, bytes) -> {
                    try {
                        ...
                        return bytes.asByteArray()
                                .map(b -> {
                                    try {
                                        ...
                                        return new Response(new ArrayList<>(response.responseHeaders().entries()),
                                ...
                                .or(Mono.fromSupplier(()->
                                        new Response(new ArrayList<>(response.responseHeaders().entries()),
                                                response.status().reasonPhrase(), response.status().code(), null)));
                    } catch (Exception e) {
                        log.atError().setCause(e).setMessage("Caught exception").log();
                        throw e;
                    }
