...
| `↑↓` / `jk` | Navigate within a list                               |
| `space`     | Toggle in multi-select (subnets)                     |
| `/`         | Filter the active list (VPC, subnets)                |
| `esc`       | Back one step / dismiss modal / cancel filter        |
| `b`         | Back to welcome (where active)                       |
| `a`         | Switch AWS profile (welcome only)                    |
| `r`         | Refresh AWS detection (welcome only)                 |
| `c`         | Copy command (review) / show command (handoff) /    |
|             |   clear resume hint (welcome)                        |
| `d`         | Toggle cost detail (review)                          |
| `f1`        | Step info overlay (wizard)                           |
| `l`         | Toggle full deploy log                               |
| `m`         | Manual entry mode (wizard VPC)                       |
| `s`         | Save & exit (wizard, on picker steps only)           |
| `ctrl+s`    | Continue (intent)                                    |
| `ctrl+k`    | Skip intent capture                                  |
| `?`         | Help overlay (page-aware key list)                   |
| `q` / `^c`  | Quit                                                 |

Save & exit (`s`) only fires on wizard steps where the cursor isn't
inside a textinput, otherwise it would conflict with typing. Resume
on next launch picks up the saved session automatically.

## CLI flags

```
migration-assistant -h, --help          show help and exit
migration-assistant -v, --version       print version and exit
migration-assistant     --debug         verbose log level
                                        (also: LOG_LEVEL=debug)
```
