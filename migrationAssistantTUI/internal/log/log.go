...
//   - On launch, the previous log (if any) is rotated to tui.log.prev so
//     post-mortem of the last-but-one run is still possible.
...
func Setup(verbose bool) (*os.File, error) {
	...
	// Rotate previous run's log to .prev — keeps one extra generation
	// for support without growing unbounded.
