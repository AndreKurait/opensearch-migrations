...
const sessionFile = ".ma-session.json"

// SaveWizardSession writes a JSON snapshot of `payload` to the workdir
// so the next launch can offer to resume the wizard mid-flow.
//
// Atomic: write-temp + rename. payload is any JSON-serializable value
// (typically wizard.State).
func SaveWizardSession(workdirPath string, payload any) error {
	if workdirPath == "" {
		return errors.New("workdir path is required")
	}
	...
	dst := filepath.Join(workdirPath, sessionFile)
	tmp := dst + ".tmp"
...
// LoadWizardSession reads a previously-saved session into out (which
// must be a pointer). Returns (false, nil) if the file is absent.
func LoadWizardSession(workdirPath string, out any) (bool, error) {
	p := filepath.Join(workdirPath, sessionFile)
	b, err := os.ReadFile(p)
	if err != nil {
