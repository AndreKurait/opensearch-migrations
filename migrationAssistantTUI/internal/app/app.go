...
type App struct {
	...
	// program — set by AttachProgram; used to push broker events into
	// tea.Program if we want a non-channel path. Optional.
	program *tea.Program
...
// AttachProgram lets the app forward broker messages directly into the
// program if it ever needs to (e.g. on shutdown, to inject a final msg).
// Optional — Workspace.Events() is the canonical pull path.
