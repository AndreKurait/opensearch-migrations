...
func TestE2E_FullHappyPath_Manual(t *testing.T) {
	...
	// 3. Skip intent.
	d.Send(tea.KeyPressMsg{Code: 'k', Text: "k"})
...
// TestE2E_DeployTickAdvances verifies the deploy page advances through
// all three phases (CFN → image mirroring → helm) via the simulated tick.
//
// The deploy page returns a tea.Tick cmd, which we manually drive by
// sending tickAdvance manually. We need access to the unexported
// tickAdvance type — work around by sending repeated TickMsgs that
// advance via spinner.Tick (which the deploy page handles).
func TestE2E_DeployTickAdvances(t *testing.T) {
	t.Parallel()
	// Direct unit test on deploy page is in deploy_test.go — see
	// internal/ui/pages/deploy/deploy_test.go for tick-advance coverage.
	t.Skip("Covered by package-private deploy_test.go test")
}
