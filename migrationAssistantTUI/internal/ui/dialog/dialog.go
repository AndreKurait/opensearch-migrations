...
// Push adds a dialog to the top. Returns the dialog's Init cmd, if any,
// so the caller can batch it into the next tea.Cmd. Callers that don't
// need Init may discard the result.
...
func (s *Stack) Update(message tea.Msg) (*Stack, tea.Cmd, Action) {
	...
	case ActionClose:
		s.Pop()
	case ActionReplace:
		rep := updated.Replacement()
		if rep == nil {
			// Replace-with-nil is a programmer error; coerce to Close so
			// the next Update doesn't panic on a nil top.
			s.Pop()
		} else {
			s.items[len(s.items)-1] = rep
		}
	default:
