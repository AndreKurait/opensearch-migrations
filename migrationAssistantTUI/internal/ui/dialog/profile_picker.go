...
//
// Prefills from $AWS_PROFILE if set, so users on a non-default profile
// see what's currently in effect and can edit instead of re-typing.
func NewProfilePicker(onSubmit func(name string) tea.Cmd) *ProfilePicker {
	t := textinput.New()
	t.Placeholder = "default"
	t.SetWidth(40)
	if cur := os.Getenv("AWS_PROFILE"); cur != "" {
		t.SetValue(cur)
	}
...
// Init focuses the input and returns the cursor-blink cmd. Called by
// the dialog stack when the picker is pushed.
