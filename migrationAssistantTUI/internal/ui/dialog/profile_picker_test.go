...
func TestProfilePickerInitReturnsFocusCmd(t *testing.T) {
	t.Parallel()
	// Init focuses the textinput; the cmd is the cursor-blink animator.
	cmd := dialog.NewProfilePicker(nil).Init()
	require.NotNil(t, cmd, "Init must return a focus/blink cmd so the cursor animates when the picker opens")
}
