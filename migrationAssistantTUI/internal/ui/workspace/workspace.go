...
type Workspace interface {
	...
	// SetGoodbye records a final line main.go prints after the alt-screen
	// tears down. Use to surface a kubectl invocation the user can re-run,
	// or a friendly "see you next time" banner. May be called any time;
	// only the most recent value is shown.
	SetGoodbye(s string)

	// SetWorkdir records the resolved named-subdir path so the post-TUI
	// handoff exec can chdir into it. Goroutine-safe in the production
	// implementation.
	SetWorkdir(s string)

	// SetAgentBin records the binary name of the user's chosen agent
	// CLI ("claude" / "kiro-cli"), so the handoff page resolves the right
	// exec target. Empty string means "no agent installed".
	SetAgentBin(s string)
