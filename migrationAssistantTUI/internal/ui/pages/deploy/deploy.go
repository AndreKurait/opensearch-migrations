...
type Model struct {
	...
	// TickInterval is unused now (real driver paces) but kept so tests
	// that override it still compile.
...
func (m *Model) Update(message tea.Msg) (*Model, tea.Cmd) {
	...
	case PhaseEventMsg:
		...
		return m, nil
	case tickAdvance:
		// Legacy v1 simulation: advance one event in the current phase. Kept
		// for tests that drive ticks directly without the real driver.
		if m.current < len(m.phases) {
			ph := &m.phases[m.current]
			if ph.Status == "PENDING" {
				ph.Status = "IN_PROGRESS"
				ph.StartedAt = time.Now()
			}
			ph.Events = append(ph.Events, fmt.Sprintf("event #%d at %s", len(ph.Events)+1, time.Now().Format("15:04:05")))
			if len(ph.Events) >= 3 {
				ph.Status = "COMPLETE"
				m.current++
				if m.current < len(m.phases) {
					m.phases[m.current].Status = "IN_PROGRESS"
					m.phases[m.current].StartedAt = time.Now()
				}
			}
		}
		if m.current >= len(m.phases) {
...
func (m *Model) View() string {
	...
	b.WriteString(s.Header.Title.Render("Deploying Migration Assistant"))
	b.WriteString("\n")
...
// tickAdvance moves the v1 simulation forward (legacy test path).
type tickAdvance struct{}

func (m *Model) advanceTick() tea.Cmd {
	d := m.TickInterval
	if d <= 0 {
		d = 800 * time.Millisecond
	}
	return tea.Tick(d, func(time.Time) tea.Msg { return tickAdvance{} })
}
