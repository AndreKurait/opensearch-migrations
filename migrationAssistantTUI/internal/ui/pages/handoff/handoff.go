...
func (m *Model) View(mode welcome.Mode, cap intent.Captured, st wizard.State) string {
	...
	case welcome.ModeAgent:
		...
		b.WriteString("\n\n")
		b.WriteString(s.Code.Render(agentCmd(st)))
	default:
	...
	b.WriteString("\n\n")
	b.WriteString(s.Footer.Hint.Render("[enter] open now   [c] show command   [s] save & exit"))
...
func kubectlCmd(st wizard.State) string {
	ns := st.Namespace
	if ns == "" {
		ns = "migration-assistant"
	}
	cluster := st.Stage
	if cluster == "" {
		cluster = "dev"
	}
	return fmt.Sprintf("kubectl --context=migration-eks-cluster-%s-%s -n %s exec -it migration-console-0 -- /bin/bash", cluster, st.Region, ns)
}

func agentCmd(st wizard.State) string {
	// v1: detect agent at exec time; here we render a deterministic hint.
