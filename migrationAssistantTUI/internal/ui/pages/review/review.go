...
type Model struct {
	...
	// blockedSt is the wizard.State the page was last asked to render. We
	// remember it so Update can consult stageConflicts on enter without
	// the host having to thread state into the key handler.
	blockedSt wizard.State
...
func (m *Model) Update(message tea.Msg) (*Model, tea.Cmd) {
	if k, ok := message.(tea.KeyPressMsg); ok {
		...
		case key.Matches(k, kk.Launch):
			...
			// already explains why; this keeps the user from burning
			// 12 minutes on a doomed deploy.
			if m.stageConflicts(m.blockedSt) {
				return m, nil
			}
			return m, func() tea.Msg { return LaunchMsg{} }
		case key.Matches(k, kk.CopyCmd):
			return m, func() tea.Msg {
				return msg.StatusMsg{Text: "Select the command above and copy with your terminal's copy shortcut.", TTL: 5 * time.Second}
			}
...
func (m *Model) View(st wizard.State) string {
	...
	b.WriteString(s.Footer.Hint.Render("[enter] launch   [b] back   [c] copy command   [d] cost details   [q] quit"))
...
func BuildArgv(st wizard.State) string {
	...
	// VPC-related flags only matter for import-vpc scope. Suppressing
	// them under create-vpc / skip-cfn keeps the equivalent argv aligned
	// with what aws-bootstrap.sh would actually accept.
	if st.Scope == "import-vpc" {
		if st.VPCID != "" {
	...
	for _, p := range st.AdditionalAccessARNs {
		b.WriteString(fmt.Sprintf("  --eks-access-principal-arn %s \\\n", p))
	}
	if st.IgnoreChecks {
