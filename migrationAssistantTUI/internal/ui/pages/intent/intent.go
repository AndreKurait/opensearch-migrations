...
// Init focuses the textarea and returns the cursor-blink cmd so the
// cursor actually animates while the user is composing the goal.
...
func (m *Model) Update(message tea.Msg) (*Model, tea.Cmd) {
	if k, ok := message.(tea.KeyPressMsg); ok {
		...
		case k.Code == tea.KeyEscape:
			// [esc] backs out to welcome without capturing intent.
			// We DON'T match Global.Back here because that also includes
			// 'b', and the textarea must allow typing 'b' in the goal.
...
func (m *Model) View() string {
	...
	b.WriteString("\n" + s.Footer.Hint.Render("[ctrl+s] continue   [ctrl+k] skip   [esc] back"))
