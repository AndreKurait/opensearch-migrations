...
func TestSkipKeyEmitsCapturedAndNavigate(t *testing.T) {
	t.Parallel()
	p := newPage(t)
	_, cmd := p.Update(tea.KeyPressMsg{Code: 'k', Mod: tea.ModCtrl})
...
func TestSkipResultIsMarkedSkipped(t *testing.T) {
	t.Parallel()
	p := newPage(t)
	_, _ = p.Update(tea.KeyPressMsg{Code: 'k', Mod: tea.ModCtrl})
	got, ok := p.Result()
	require.True(t, ok)
	require.True(t, got.Skipped)
}

func TestEscapeRoutesToWelcome(t *testing.T) {
	t.Parallel()
	p := newPage(t)
	...
	require.Equal(t, msg.PageWelcome, nav.To)
}

func TestTypingBDoesNotNavigateBack(t *testing.T) {
	t.Parallel()
	p := newPage(t)
	// Typing 'b' should reach the textarea (not Global.Back which includes 'b').
	_, cmd := p.Update(tea.KeyPressMsg{Code: 'b', Text: "b"})
	if cmd != nil {
		out := cmd()
		_, isNav := out.(msg.NavigateMsg)
		require.False(t, isNav, "[b] in intent must not navigate — would break typing")
	}
}
