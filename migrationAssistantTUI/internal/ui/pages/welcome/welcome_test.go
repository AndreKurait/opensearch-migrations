...
func TestEnterRoutesToHandoffWhenInstalled(t *testing.T) {
	t.Parallel()
	p := newPage(t)
	p, _ = p.Update(msg.WorkdirDetectedMsg{Has: true, Status: "installed", Workdir: "/tmp/wd"})
	_, cmd := p.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
	require.NotNil(t, cmd)
...
func TestEnterRoutesToReviewWhenFailedOrInProgress(t *testing.T) {
	t.Parallel()
	for _, status := range []string{"failed", "in_progress"} {
		p := newPage(t)
		p, _ = p.Update(msg.WorkdirDetectedMsg{Has: true, Status: status, Workdir: "/tmp/wd"})
		_, cmd := p.Update(tea.KeyPressMsg{Code: tea.KeyEnter})
		require.NotNil(t, cmd)
		nav, ok := cmd().(msg.NavigateMsg)
		require.True(t, ok)
		require.Equalf(t, msg.PageReview, nav.To, "status=%q must route to review", status)
	}
}

func TestCKeyClearsResumeHint(t *testing.T) {
	t.Parallel()
	p := newPage(t)
	p, _ = p.Update(msg.WorkdirDetectedMsg{Has: true, Status: "installed", Workdir: "/tmp/wd"})
	require.Contains(t, stripANSI(p.View()), "/tmp/wd")
	p, _ = p.Update(tea.KeyPressMsg{Code: 'c', Text: "c"})
	require.NotContains(t, stripANSI(p.View()), "/tmp/wd", "[c] must clear the resume hint")
}
