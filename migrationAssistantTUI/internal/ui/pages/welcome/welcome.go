...
// re-run detection. The actual profile name is captured by the
// profile-picker dialog the root pushes in response, not on this msg.
...
type Model struct {
	...
	// Resume prompt state. resumeStage is reserved for a future
	// "Resume install at stage X" hint and intentionally unused in v1.
...
func (m *Model) Update(message tea.Msg) (*Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(v, m.c.Keys.Welcome.Choose):
			// Resume routing — the user pre-confirmed they want to act on
			// the existing workdir, so skip the intent capture step.
			if m.hasResume {
				switch m.resumeStatus {
				case "installed":
					// Configured + helm release live. Hand off straight away.
					return m, func() tea.Msg { return msg.NavigateMsg{To: msg.PageHandoff} }
				case "failed", "in_progress":
					// Mid-deploy interrupted. Skip intent, jump to review so
					// the user can re-launch with the saved wizard state.
		...
		// Resume-shortcut keys: only meaningful when we have a workdir to act on.
		if m.hasResume {
			switch v.String() {
			case "c":
				m.hasResume = false
	...
	if m.hasResume {
		b.WriteString(s.Status.Warn.Render(
			"  ⚠ Existing install detected at " + m.resumeWorkdir + " (status: " + m.resumeStatus + ")\n" +
				"    Pressing [enter] resumes; press [c] to clear the resume hint.") + "\n\n")
	}
...
func (m *Model) detectionLines() string {
	...
	if !m.awsReady {
		...
		if len(m.maExports) == 0 {
			...
			stages := feature.StagesFromExports(m.maExports, m.identity.Region)
	...
	if !m.agentsReady {
		...
		if first {
			lines = append(lines, s.Status.Warn.Render(
				"  AI agents      (none detected — Manual mode still works; install kiro-cli or claude to use Agent mode)"))
		}
	}
...
// terminal row without wrapping mid-line. Operates on runes so it
// can't split a multibyte UTF-8 sequence.
