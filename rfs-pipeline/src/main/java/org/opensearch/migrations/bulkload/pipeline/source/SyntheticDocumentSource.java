...
/**
 * A synthetic DocumentSource for testing the writing side without any real snapshot.
 *
 * This is the key enabler for N+M testing: target-side tests use SyntheticDocumentSource
...
public class SyntheticDocumentSource implements DocumentSource {
    ...
    public Flux<DocumentChange> readDocuments(ShardId shardId, int startingDocOffset) {
        return Flux.range(startingDocOffset, docsPerShard - startingDocOffset)
            .map(docNum -> {
